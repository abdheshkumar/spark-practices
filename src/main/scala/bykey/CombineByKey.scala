...
object CombineByKey extends Boot {
  ...
  val students: RDD[(String, StudentDetails)] = spark.sparkContext.parallelize(List(
...
*/
  ...
  val createCombiner = (student: StudentDetails) => {
    println(s"Combiner:for student ${student.studentName}:::${(student.score, 1)}")
    (student.score, 1)
  }
 ...
 val resultOfCombine: RDD[(String, (Float, Int))] = students.combineByKey(createCombiner, mergeValue, mergeCombiners)
